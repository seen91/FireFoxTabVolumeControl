...
function initializeAudioContext() {
  ...
  try {
    ...
    if (state.audioContext.state === 'suspended') {
      ...
      setupAudioContextResumeHandling();
    }
 ...
 */
function setupAudioContextResumeHandling() {
  ...
  const mediaControlObserver = new MutationObserver((mutations) => {
    for (const mutation of mutations) {
      if (mutation.type === 'attributes' || mutation.type === 'childList') {
        ...
        findAndAttachToMediaControls(mutation.target);
      }
    }
  });
  ...
  setTimeout(() => {
    ...
    findAndAttachToMediaControls(document.body);
  }, 1000);
 ...
 */
function findAndAttachToMediaControls(rootElement) {
